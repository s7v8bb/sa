...
return send(msg.chat_id,msg.id,"انت حمار يبني تبغا تتزوج نفسك ؟!!","md",true)
end
if tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني ياورت مالقيت غيري؟ ","md",true)
end
if Redis:sismember(Fast..msg.chat_id.."zwgat:",rep_idd) then
local rd_mtzwga = {
"اسف يقلبي متزوجه",
"متزوجه يبن عمي شفلك واحده تانية",
"ياخي افهم عندي زوج واسمه جاسم💋",
"للاسف متزوجه دور وحده ثانيه ",
"ابوك يالمصيبه طلعت متزوجه قبلك"
}
...
"لولولولويي تم الزواج عقبال العيال ",
"مبروك اتزوجتها تبغا اتغدا بقا في الفرح",
"تم زواجكم... ودا رقمي عشان لو العريس معرفش يسد 012345..",
"الزواج تم اتفضلو اعملو احلا واحد يهيهي"
}
...
local UserInfo = bot.getUser(rep_idd)
if tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"احا هو انت كنت اتزوجت نفسك عشان تطلق","md",true)
end
if tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"هو احنا كنا اتزوجنا يروح خالتك عشان نطلق","md",true)
end
if Redis:sismember(Fast..msg.chat_id.."zwgat:",rep_idd) then
...
local rd_tmtlaq = {
"تم طلاقكم للاسف",
"تم الطلاق  ام عبير تبغاه تعرف اتطلقتو لي ؟",
"تم الطلاق عشان المعلم ",
"تم الطلاق عشان في سوسه دخلت وسطهم",
"تم الطلاق شوفي هذا رقمي عشان لو حبيتي نتكلم باحترام 01234..."
}
return send(msg.chat_id,msg.id,Reply_Status(rep_idd,rd_tmtlaq[math.random(#rd_tmtlaq)]).Reply,"md",true)
else
local rd_tlaq = {
"مكنت متزوج عشان تطلق اصلا",
"محد عبرها قبل  احسسن"
}
...
return send(msg.chat_id,msg.id,"انت حمار يبني تبغا ترفع نفسك فقلبك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن  اسف انفعلت","md")
...
elseif tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انت حمار يبني تبغا تنزل نفسك من قلبك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن  اسف انفعلت","md")
...
return send(msg.chat_id,msg.id,"اقعد يعم انت محد يبيك","md")
...
return send(msg.chat_id,msg.id,"ماعندنا احد للزواج قفلنا خذلك كلبه َوفكنا","md")
end
...
return send(msg.chat_id,msg.id,"جبتلك عروسه جبتها لك من حراج تمشي ولا اغير؟ صدقني ذوقي حلو "..mem_tag.." ايشرايك فيها ؟؟","md",false, false, false, false, reply_markup)
end
end
...
return send(msg.chat_id,msg.id,"منتو متزوجين ولا هو محن وخلاص","md")
elseif tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انت حمار يبني تبغا تتزوج نفسك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن الكلب","md")
...
return send(msg.chat_id,msg.id,"بنادي لك زوجها\n["..FlterBio(zwg_info.first_name).."](tg://user?id="..zwg_id..")\nالحق يا دكر يبغون يتزوجو زوجتك","md")
elseif Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
local zwg_id = Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")
local zwg_info = bot.getUser(zwg_id)
return send(msg.chat_id,msg.id,"هناديلك زوجتك\n["..FlterBio(zwg_info.first_name).."](tg://user?id="..zwg_id..")\nالحقي يا وليه زوجك تبغا يتجوز عليكي","md")
...
return send(msg.chat_id,msg.reply_to_message_id,"يا "..rep_tag.."\nالحلو"..user_tag.."\nطالب ايدك للزواج ايشرايك ؟","md",false, false, false, false, reply_markup)
end
end
if text == "زوجتي" then
...
return send(msg.chat_id,msg.id,"كلمي يا ["..zwga_name.."](tg://user?id="..zwga_id..") زوجك تبغاك","md")
elseif not Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
return send(msg.chat_id,msg.id,"زوجتك ايشي انت سنقل","md")
end
end
if text == "زوجي" then
if Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
local zwga_id = Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")
local zwga_name = FlterBio(bot.getUser(zwga_id).first_name)
return send(msg.chat_id,msg.id,"كلم يا ["..zwga_name.."](tg://user?id="..zwga_id..") زوجتك تبيك","md")
elseif not Redis:get(Fast..msg.chat_id..senderr.."mtzwga:") then
return send(msg.chat_id,msg.id,"انتي مو متزوجه يا ام كشه","md")
end
end
if text == "انتي طالق" and msg.reply_to_message_id ~= 0 then 
local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
if tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انت حمار يبني تبغا تطلق نفسك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعد عني يبن الكلب","md")
...
return send(msg.chat_id,msg.id,"مو زوجتك عشان تطلقها يحمار","md")
elseif tonumber(Redis:get(Fast..msg.chat_id..rep_idd.."mtzwga:")) == tonumber(senderr) or tonumber(Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")) == tonumber(rep_idd) then
Redis:del(Fast..msg.chat_id..rep_idd.."mtzwga:")
Redis:del(Fast..msg.chat_id..senderr.."mtzwga:")
return send(msg.chat_id,msg.id,"تم طلاقكم وشوفو العيال  مع مين","md")
end
end
if text == "انت طالق" and msg.reply_to_message_id ~= 0 then 
local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
if tonumber(rep_idd) == tonumber(senderr) then
return send(msg.chat_id,msg.id,"انتي هبله يبنتيي تبغين تطلقين نفسك ؟؟","md")
elseif tonumber(rep_idd) == tonumber(Fast) then
return send(msg.chat_id,msg.id,"ابعدي عني يبنت الكلب","md")
elseif tonumber(Redis:get(Fast..msg.chat_id..rep_idd.."mtzwga:")) ~= tonumber(senderr) or tonumber(Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")) ~= tonumber(rep_idd) then
return send(msg.chat_id,msg.id,"مو زوجك يحماره عشان تطلقي منو","md")
elseif tonumber(Redis:get(Fast..msg.chat_id..rep_idd.."mtzwga:")) == tonumber(senderr) or tonumber(Redis:get(Fast..msg.chat_id..senderr.."mtzwga:")) == tonumber(rep_idd) then
Redis:del(Fast..msg.chat_id..rep_idd.."mtzwga:")
Redis:del(Fast..msg.chat_id..senderr.."mtzwga:")
return send(msg.chat_id,msg.id,"تم طلاقكم وشوفو العيال مع مين","md")
