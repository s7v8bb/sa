...
os.execute('chmod +x Snak;chmod +x Snak;./Snak')
end
...
bot = Fx.Snakbots.set_config{api_id=4911262,api_hash='ca0c325a23f1e45f2862ca6fc6bd6dec',session_name=Snak,token=Token}
function var(value)
...
function Dev(msg) 
if Redis:sismember(Snak.."Dev",msg.sender_id.user_id) or tonumber(msg.sender_id.user_id) == tonumber(Sudo_Id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender_id.user_id) == tonumber(Snak) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(Snak.."chfalse")
if chh then
...
if data.sender_id.user_id == tonumber(Snak) then
return false
end
if Redis:get(Snak.."chsource") then
chsource = Redis:get(Snak.."chsource")
else
chsource = "otlop12"
end
...
local Info_User = Redis:get(Snak.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(Snak..'BaN:In:Tuasl',Info_User)  
return send(data.chat_id,data.id,Reply_Status(Info_User,'⌯ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(Snak..'BaN:In:Tuasl',Info_User)  
return send(data.chat_id,data.id,Reply_Status(Info_User,'⌯ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "⌯ الغاء الامر" then
Redis:del(Snak..data.sender_id.user_id.."bottoken")
Redis:del(Snak..data.sender_id.user_id.."botuser")
Redis:del(Snak..data.sender_id.user_id.."make:bot")
...
Redis:del(Snak.."free:bot")
send(data.chat_id,data.id,'⌯ تم تفعيل الوضع المجاني ',"md",true)  
end
if text == "⌯ تعطيل الوضع المجاني" then 
Redis:set(Snak.."free:bot",true)
send(data.chat_id,data.id,'⌯ تم تعطيل الوضع المجاني ',"md",true)  
end
-----تشغيل البوتات ---
if text and Redis:get(Snak..data.sender_id.user_id.."run:bot") then
Redis:del(Snak..data.sender_id.user_id.."run:bot")
Redis:del(Snak.."screen:on")
Redis:del(Snak.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
Redis:sadd(Snak.."bots:folder",folder:gsub("@",""))
end
end
if not Redis:sismember(Snak.."bots:folder",userbot) then
...
Redis:sadd(Snak.."screen:on",screen)
end
local list = Redis:smembers(Snak..'screen:on')
...
Redis:set(Snak..data.sender_id.user_id.."run:bot",true)
send(data.chat_id,data.id,"⌯ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(Snak..data.sender_id.user_id.."stop:bot") then
Redis:del(Snak..data.sender_id.user_id.."stop:bot")
Redis:del(Snak.."screen:on")
Redis:del(Snak.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(Snak..data.sender_id.user_id.."stop:bot",true)
send(data.chat_id,data.id,"⌯ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(Snak.."ch:addd"..data.sender_id.user_id) == "on" then
Redis:set(Snak.."ch:addd"..data.sender_id.user_id,"off")
...
Redis:del(Snak.."chfalse")
Redis:set(Snak.."chfalse",ch)
Redis:del(Snak.."ch:admin")
Redis:set(Snak.."ch:admin",da.result.invite_link)
else
send(data.chat_id,data.id,'⌯ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "⌯ تفعيل الاشتراك الاجباري" then
Redis:set(Snak.."ch:addd"..data.sender_id.user_id,"on")
...
if text and Redis:get(Snak..data.sender_id.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(Snak..data.sender_id.user_id.."bottoken")
local botuser = Redis:get(Snak..data.sender_id.user_id.."botuser")
...
Redis:sadd(Snak.."userbots",botuser)
Redis:del(Snak..data.sender_id.user_id.."bottoken")
Redis:del(Snak..data.sender_id.user_id.."botuser")
Redis:del(Snak..data.sender_id.user_id.."make:bot")
...
if text and Redis:get(Snak..data.sender_id.user_id.."make:bot") == "token" then
...
if Redis:sismember(Snak.."userbots",botuser) then
send(data.chat_id,data.id, "\n⌯ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(Snak..data.sender_id.user_id.."botuser",botuser)
Redis:set(Snak..data.sender_id.user_id.."bottoken",text)
Redis:set(Snak..data.sender_id.user_id.."make:bot","devuser")
...
Redis:set(Snak..data.sender_id.user_id.."make:bot","token")
...
if text == "⌯ فحص" then
Redis:del(Snak.."All:pv:st")
Redis:del(Snak.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
if Redis:get(folder) then
m = Redis:get(folder)
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(Snak.."All:gp:st",tonumber(#list))
Redis:incrby(Snak.."All:pv:st",tonumber(#lt))
end
end
end
send(data.chat_id,data.id,'\n⌯ احصائيات جميع البوتات المصنوعه \n ⌯ عدد المجموعات '..Redis:get(Snak.."All:gp:st")..' مجموعه \n⌯ عدد المشتركين '..Redis:get(Snak.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
Redis:del(Snak..'3ddbots')
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
os.execute('cd '..folder..' && chmod +x * && screen -d -m -S '..folder:gsub("@","")..' ./Run')
Redis:sadd(Snak..'3ddbots',folder)
end
end
local list = Redis:smembers(Snak..'3ddbots')
...
-------delete 
if text and Redis:get(Snak..data.sender_id.user_id.."make:bot") == "del" then
...
Redis:srem(Snak.."userbots",text:gsub("@",""))
send(data.chat_id,data.id, "\n⌯ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "⌯ حذف بوت" then
Redis:set(Snak..data.sender_id.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(Snak..'3ddbots',botuser..' » '..devbot)
end
end
end
local list = Redis:smembers(Snak..'3ddbots')
...
----end--3dd
if text and Redis:get(Snak..data.sender_id.user_id.."setchannel") then
if text == "الغاء" or text == '⌯ الغاء الامر' then   
Redis:del(Snak..data.sender_id.user_id.."setchannel")
send(data.chat_id,data.id, "\n⌯ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(Snak.."chsource",ch)
...
Redis:set(Snak..data.sender_id.user_id.."setchannel",true)
...
Redis:set(Snak.."twsl",true)
send(data.chat_id,data.id,"⌯ تم تعطيل التواصل")
return false 
end
if text == "⌯ الاحصائيات" then
local list = Redis:smembers(Snak.."total")
...
if tonumber(Snak) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'⌯ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(Snak..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(Snak.."total")
local Get_Json = '{"BotId": '..Snak..','  
if #UsersBot ~= 0 then 
...
----brodcast all
if Redis:get(Snak.."all:texting") then
if text == "الغاء" or text == '⌯ الغاء الامر' then   
Redis:del(Snak.."all:texting")
send(data.chat_id,data.id, "\n⌯ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(Snak.."3z:gp",text)
Redis:del(Snak.."all:texting")
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(Snak.."3z:gp")).."&parse_mode=Markdown")
end
end
end
end
Redis:del(Snak.."3z:gp")
Redis:del(Snak.."all:texting")
send(data.chat_id,data.id,"⌯ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(Snak.."all:texting:pv") then
if text == "الغاء" or text == '⌯ الغاء الامر' then   
Redis:del(Snak.."all:texting:pv")
send(data.chat_id,data.id, "\n⌯ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(Snak.."eza3a:pv",text)
Redis:del(Snak.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(Snak.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
end
Redis:del(Snak.."eza3a:pv")
Redis:del(Snak.."all:texting:pv")
send(data.chat_id,data.id,"⌯ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "⌯ اذاعه عام للمجموعات" then
Redis:set(Snak.."all:texting",true)
send(data.chat_id,data.id,"ارسل النص الان","html",true)
end
if text == "⌯ اذاعه عام للمشتركين" then
Redis:set(Snak.."all:texting:pv",true)
send(data.chat_id,data.id,"ارسل النص الان","html",true)
end
--brodcast
if Redis:get(Snak..data.sender_id.user_id.."brodcast") then 
if text == "الغاء" or text == '⌯ الغاء الامر' then   
Redis:del(Snak..data.sender_id.user_id.."brodcast") 
send(data.chat_id,data.id, "\n⌯ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(Snak.."total") 
...
if text == "⌯ اذاعه" then
Redis:set(Snak..data.sender_id.user_id.."brodcast",true)
send(data.chat_id,data.id,"⌯ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(Snak..data.sender_id.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '⌯ الغاء الامر' then   
Redis:del(Snak..data.sender_id.user_id.."brodcast:fwd")
...
Redis:set(Snak..data.sender_id.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(Snak.."ch:admin")
var(bot.sendText(msg.chat_id,msg.id,'\n⌯ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..''))
return false 
end
if not Redis:get(Snak.."twsl") then
if msg.sender_id.user_id ~= tonumber(Snak) then
if Redis:sismember(Snak..'BaN:In:Tuasl',msg.sender_id.user_id) then
return false 
end
if msg.id then
Redis:setex(Snak.."Twasl:UserId"..msg.date,172800,msg.sender_id.user_id)
...
if text and Redis:get(Snak.."free:bot") then
return send(msg.chat_id,msg.id,"⌯ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(Snak.."total",msg.sender_id.user_id) then
Redis:sadd(Snak.."total",msg.sender_id.user_id)
end
reply_markup = bot.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id," ⋆ مرحبا بك في صانع بوتات حمايه المجاني.\n⋆ قم بتنصيب بوت مجاني الان وبحقوقك انت وقناتك للاستفسار @qo_nl","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(Snak..msg.sender_id.user_id.."make:bot") then
if text == "⌯ الغاء" then
Redis:del(Snak..msg.sender_id.user_id.."make:bot")
...
if Redis:sismember(Snak..'fake',botuser) then
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(Snak..msg.sender_id.user_id.."my:bot",botuser)
Redis:sadd(Snak.."userbots",botuser)
Redis:del(Snak..msg.sender_id.user_id.."make:bot")
...
if text == "⌯ صنع بوت" thenas
if Redis:get(Snak..msg.sender_id.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"⌯ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(Snak..msg.sender_id.user_id.."make:bot",true)
...
if Redis:get(Snak..msg.sender_id.user_id.."my:bot") then
local botuser = Redis:get(Snak..msg.sender_id.user_id.."my:bot")
...
Redis:srem(Snak.."userbots",botuser)
Redis:del(Snak..msg.sender_id.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n⌯ تم حذف البوت بنجاح","md",true)  
else
...
if data and data.Snakbots and data.Snakbots == "updateNewMessage" then
if tonumber(data.message.sender_id.user_id) == tonumber(Snak) then
return false
end
Run(data.message,data.message)
elseif data and data.Snakbots and data.Snakbots == "updateMessageEdited" then
local Message_Edit = bot.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender_id.user_id == Snak then
return false
end
Run(Message_Edit,Message_Edit)
elseif data and data.Snakbots and data.Snakbots == "updateNewCallbackQuery" then
